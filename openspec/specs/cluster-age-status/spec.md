...
## Purpose
Derives a four-tier recency for each cluster from its creation age and known activity recency, so operators can spot clusters that were likely created and then forgotten, without misflagging long-lived clusters that are still in active use.
## Requirements
