...
- [x] 7.1 Cut a real version tag (`v0.1.0-test.1`, with the user's explicit go-ahead), pushed to `origin`. `.github/workflows/release.yml` ran successfully on GitHub (`conclusion: success`) and created a real GitHub Release with `capella-housekeeper-0.1.0.tgz` attached (13.2 MB, matching the local build). Fully closed the loop: ran `npx <the actual published Release download URL>` (not a local file) from a clean `$HOME`, and confirmed the dashboard starts, the login proxy redirect works, and the sync scheduler/Slack bot start correctly - the real end-to-end path a user would actually take.
